...
def generate_circular_trajectory(x0, T, dt, center=(0.0, 0.0), radius=None):
    cx, cy = center
    x0, y0, th0 = x0
    if radius is None:
        radius = np.hypot(x0 - cx, y0 - cy)
    ang0  = np.arctan2(y0 - cy, x0 - cx)
    times = np.arange(0.0, T, dt)
    traj = []
    for t in times:
        ang   = ang0 + 2*np.pi * (t / T)
        x     = cx + radius * np.cos(ang)
        y     = cy + radius * np.sin(ang)
        theta = ang + np.pi/2
        traj.append(np.array([x, y, theta], dtype=float))
    return traj
